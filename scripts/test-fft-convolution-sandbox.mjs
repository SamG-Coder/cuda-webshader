...
try{
 ...
 const url=process.env.CW_PUBLIC_URL||`http://127.0.0.1:${server.address().port}/sandbox.html?example=fft-convolution`;await page.goto(url);
 ...
 const report=await page.evaluate(async()=>{
  const s=window.sandbox,r=s.pipelineResult,native=new Float32Array(await(await fetch('reports/fft-convolution-1-output.bin')).arrayBuffer());
 ...
 const source=await readFile('showcases/fft-convolution/kernel.cu','utf8');if((await page.evaluate(()=>window.sandbox.editor.getValue())).replaceAll('\r\n','\n')!==source.replaceAll('\r\n','\n'))throw Error('Original source changed');
 await page.locator('#tab-compare').click();const passes=await page.locator('#shader-pass option').allTextContents();for(const name of ['padKernel_kernel','padDataClampToBorder_kernel','modulateAndNormalize_kernel','forwardFftAxis','inverseFftAxis'])if(!passes.some(p=>p.includes(name)))throw Error('Missing shader '+name);
 await page.screenshot({path:'.local/fft-convolution-sandbox.png'});await page.setViewportSize({width:390,height:844});if(!await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth))throw Error('Mobile overflow');
 ...
 await writeFile('reports/fft-convolution-sandbox-check.json',JSON.stringify({...report,sourceUnchanged:true,passes,mobileOverflow:false,presetSwitching:true},null,2));console.log(JSON.stringify(report,null,2));
