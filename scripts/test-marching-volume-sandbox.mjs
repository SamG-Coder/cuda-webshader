...
try{
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/sandbox.html?example=marching-volume`);
 ...
 const result=await page.evaluate(async()=>{
  ...
  const comparisons=[];for(const [name,resource]of [['positions','pos'],['normals','norm']]){const actual=await s.runtime.read(r.buffers[resource],Float32Array,r.count*16),expected=new Float32Array(await(await fetch('/reports/marching-volume-native-1-'+name+'.bin')).arrayBuffer());if(actual.length!==expected.length)throw Error('Native capture size mismatch');let maxError=0;for(let i=0;i<actual.length;i++){if(!Number.isFinite(actual[i]))throw Error('Nonfinite '+name);maxError=Math.max(maxError,Math.abs(actual[i]-expected[i]));}if(maxError>1e-6)throw Error(name+' mismatch '+maxError);comparisons.push({name,components:actual.length,maxError});}
  if(r.count!==33378||r.controlReadbackBytes!==8)throw Error('Unexpected vertex count or control transfers');return {device:s.runtime.describe(),vertices:r.count,triangles:r.count/3,controlReadbackBytes:r.controlReadbackBytes,comparisons};
 });
 ...
 await page.screenshot({path:'reports/marching-volume-sandbox.png'});
 ...
 await writeFile('reports/marching-volume-sandbox-check.json',JSON.stringify({passed:true,staticBuild:true,softwareAdapterRequested:false,...result,passes,sourceUnchanged:true,orbitWithoutComputeOrDataTransfer:true,emptySurfacePassed:true,mobileOverflow:false},null,2));
 console.log('PASS marching-cubes Bucky sandbox',JSON.stringify(result));
