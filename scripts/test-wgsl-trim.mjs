...
try{
 ...
 const result=await page.evaluate(async ({steps,comparisonMode,scalingMode,specializeMode,reverse})=>{
  ...
  const modes=specializeMode?['trimmed','specialized','specialized','trimmed']:scalingMode?['trimmed','scaling','scaling','trimmed']:comparisonMode?['trimmed','comparisons','comparisons','trimmed']:['original','trimmed','trimmed','original'];
  for(const mode of reverse?[modes[1],modes[0],modes[3],modes[2]]:modes){
   ...
   try{
    ...
    const kernelFactory=async(source,options)=>{
     ...
     const optimized=mode==='specialized'?specializeIntegerUniforms(trimmed.artifact,params):mode==='scaling'?optimizeFloatScaling(trimmed.artifact):mode==='comparisons'?optimizeFloatComparisons(trimmed.artifact):null,trimMs=performance.now()-time,selected=optimized?.artifact??trimmed?.artifact??artifact;
 ...
 },{steps,comparisonMode:process.argv.includes('--comparisons'),scalingMode:process.argv.includes('--scaling'),specializeMode:process.argv.includes('--specialize'),reverse:process.argv.includes('--reverse')});
