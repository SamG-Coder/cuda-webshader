...
const rows=JSON.parse(await readFile('reports/nvidia-artifacts.json','utf8')).filter(r=>r.sample!=='cpp/6_Performance/transpose');
for(const entry of ['transposeCoalesced','transposeNoBankConflicts']){
...
await writeFile('reports/nvidia-artifacts.json',JSON.stringify(rows));console.log('Compiled NVIDIA coalesced and padded transpose kernels.');
