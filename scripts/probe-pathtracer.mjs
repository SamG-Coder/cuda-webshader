...
for(const file of ['main.cu','vec3.h','vec3-device-only','ray.h','hitable.h']) {
 ...
 if(file==='ray.h'){const vec=await readFile(new URL('vec3.h',root),'utf8');candidate=vec.replace(/^\s*#.*$/gm,'').replace(/inline std::(?:istream|ostream)& operator[<>]{2}\([^)]*\)\s*\{[^}]*\}/g,'')+'\n'+candidate;}
