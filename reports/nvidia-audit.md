...
208 sample directories inventoried; 350 direct-source CUDA entry probes. 38 isolated kernels passed native CUDA and real NVIDIA WebGPU reference checks. These use 256 elements, a 32×32 grid, 64×64 transpose/multiplication matrices, 17 dot products of 1,537 elements, 256 BlackScholes options, 3,072 unsigned scan-update values, 1,024 threads updating ten CAS counters, 257 records in built-in typed copies, 3,072 values in a batched Walsh-transform pass, three complete 256-value shared-memory transforms, merges of 17 partial 64/256-bin histograms, 64 inverse-normal values, or 256 square roots. The absolute error limit is 0.000003 except BlackScholes (0.0002 absolute plus 0.00002 relative against an independent normal-distribution reference) and inverseCND (0.00002 absolute against independent normal-tail quadrature), and square roots (0.0000002 relative, no absolute floor). They are correctness checks, not performance measurements.
...
All 38 supported isolated kernels passed on a real NVIDIA adapter. The initial direct-source scan rejected 120 sample directories; each exact compiler error is recorded in nvidia-audit.json. 38 have no extractable standalone entry (including library/host-only/template examples). 34 Python samples are not CUDA C input. The baseline had 18 translated entries from 16 directories; the transpose follow-up adds two verified entries, scalarProd adds one, BlackScholes adds its complete kernel file with two helpers, and matrixMul adds 16- and 32-wide integer template specializations. The scan follow-up adds its uniformUpdate stage; helper shared-memory pointers still block earlier scan stages. The atomic follow-up adds the original cas_atomic retry loop. The alignedTypes follow-up adds int, uint4 and float4 specializations of its copy template; custom alignment structs remain unsupported. The FWT follow-up adds its global-memory radix-4 pass; the subsequent dynamic-shared-memory follow-up also verifies its finishing kernel on complete transforms up to 2,048 values. Both histogram merge stages are also verified; their byte-counting kernels remain unsupported. The inverseCND kernel and original Moro helper are verified in required-input-buffer mode; null input is rejected. The original naive transpose is also verified alongside the tiled and padded variants. The simpleMPI square-root compute kernel is verified; MPI communication is not implemented in the browser. The complete simpleDrvRuntime vector-add kernel file now compiles with its original C linkage declaration; host driver/runtime interoperability remains outside the browser. The bitonic global merge and original reference-parameter comparator are verified on unsigned key/value pairs; the sandbox preset runs one stage. Baseline errors describe the original import attempts, while current GPU checks describe the supported extracted kernels.
