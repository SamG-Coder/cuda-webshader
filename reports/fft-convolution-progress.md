...
## Complete custom test1 and fused test2

Both original-size variants now run end to end with GPU-only texture transfers. Full outputs and all input-domain spatial convolution pixels pass against native CUDA and the original CPU criterion. Test1 relative native L2 is 2.0247303e-7; test2 is 2.0239125e-7. Smaller non-square inputs also pass. See fft-custom-full-check.json. Each variant has its own showcase card and sandbox preset, exposing original CUDA alongside all generated stages.

Generic runtime support adds float/uint buffer-to-linear-texture row copies with source offset and partial-tail bounds. The sandbox adds explicit bounded byte reinterpretation copies and forward/inverse complex FFT steps; existing inverseFFT and realFFT steps remain supported. The fused host pipeline uses a distinct output buffer to avoid WebGPU writable aliases, retaining all original device bodies. No convolutionFFT2D test0/test1/test2 path remains unimplemented after this milestone.
