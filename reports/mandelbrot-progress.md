...
With the original header and CUDA file extracted together, Mandelbrot0<float> now gets past the kernel signature. The next measured compiler error is the chained reference assignment c0 = e = t1 + t2 in dsadd. The emitter currently accepts chains only through named locals and emits ordinary helpers even when the selected kernel does not call them. Further original-source requirements include per-component writes to packed storage pixels in accumulated frames and const references to packed colour records. These need correct assignment and storage semantics before claiming the original renderer works. The secondary anti-alias pass and double-single path require separate validation; no claim of double-precision WebGPU support is made.

Next: implement and verify the generic assignment/storage requirements without changing NVIDIA function bodies, then compare rendered images against native CUDA before exposing a sandbox showcase.
