...
int main(){
 ...
 FILE* input=fopen("showcases/smoke/positions.bin","rb");if(!input)return 2;got=fread(pos.data(),16,n,input);fclose(input);if(got!=n)return 2;
 input=fopen("showcases/smoke/velocities.bin","rb");if(!input)return 2;got=fread(vel.data(),16,n,input);fclose(input);if(got!=n)return 2;
 ...
 SimParams params={};params.gravity=make_float3(0,0,0);params.globalDamping=1;params.noiseFreq=.1f;params.noiseAmp=.001f;params.noiseSpeed=make_float3(0,0,0);
 for(unsigned frame=1;frame<=64;frame++){
  ...
  if(frame==64){
   ...
   char path[160];snprintf(path,sizeof(path),"reports/smoke-showcase-%u-positions.bin",frame);if(!save(path,pos.data(),n*16))return 2;
   snprintf(path,sizeof(path),"reports/smoke-showcase-%u-velocities.bin",frame);if(!save(path,vel.data(),n*16))return 2;
   snprintf(path,sizeof(path),"reports/smoke-showcase-%u-depth.bin",frame);if(!save(path,keys.data(),n*4))return 2;
