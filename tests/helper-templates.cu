...
template<class T> __device__ T nested(T x) { return square<T>(x)+x; }
...
__global__ void helperTemplates(float* out,unsigned int* bits,unsigned int n) {
 ...
 if(i<n){float x=((int)(i%13u)-6)*0.25f;float a=x;float b=2.0f;exchange<float>(a,b);
 float4 v=identity<float4>(make_float4(x,x+1.0f,x+2.0f,x+3.0f));
 out[i*4u]=nested<float>(x);out[i*4u+1u]=repeat<3>(x);out[i*4u+2u]=a;out[i*4u+3u]=v.y;
 bits[i]=nested<uint>(0x80000000u+i);
 }
}
