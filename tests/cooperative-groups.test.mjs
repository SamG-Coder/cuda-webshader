...
test('Helpers may create a local block handle and synchronize',()=>assert.match(compile(prefix+'__device__ void helper(){blocks::thread_block team=blocks::this_thread_block();blocks::sync(team);} __global__ void k(){helper();}').wgsl,/workgroupBarrier/));
