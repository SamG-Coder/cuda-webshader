...
export async function createChronoRebuild(runtime,params,n,{cudaSource,kernelFactory}={}){
 ...
 async function search(buffers,activeList,n){
  ...
  const dispatch=(entry,args)=>{if(!n)return;const k=kernels[entry],values=Object.fromEntries(k.artifact.metadata.scalars.map(s=>[s.name,s.origin==='constant'?p[s.name]:n]));(pendingBatch??=runtime.batch()).dispatch(bind(k,args,values),[Math.ceil(n/128)]);};
