...
int main(){
 const int dimensions[][2]={{2048,8},{8,2048},{2048,2048}};
 for(const auto& dims:dimensions){
  ...
  char path[160];snprintf(path,sizeof(path),"reports/large-fft-%dx%d-input.bin",width,height);if(!save(path,data.data(),n*4))return 2;
  ...
  FFT(cufftExecR2C(forward,gpu,(cufftComplex*)gpu));CUDA(cudaMemcpy(data.data(),gpu,n*4,cudaMemcpyDeviceToHost));snprintf(path,sizeof(path),"reports/large-fft-%dx%d-spectrum.bin",width,height);if(!save(path,data.data(),n*4))return 2;
  ...
  snprintf(path,sizeof(path),"reports/large-fft-%dx%d-inverse.bin",width,height);if(!save(path,compact.data(),compact.size()*4))return 2;
