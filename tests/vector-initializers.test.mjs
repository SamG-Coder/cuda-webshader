...
test('Vector aggregate initializers reject excess elements, implicit conversions and unsupported aggregates',()=>{
 for(const declaration of ['float2 a={1.0f,2.0f,3.0f};','float2 a={16777217,2};','int2 a={1.0f};','float a={1.0f};','float2 a[2]={{1.0f,2.0f}};','__shared__ float2 a={};'])assert.throws(()=>compile('__global__ void k(){'+declaration+'}'),/initializer|initializers/i);
});
