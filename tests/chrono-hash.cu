...
// MIT capture wrapper around the unchanged helpers above.
__global__ void hashProbe(const Real3* points,uint* hashes,int* bins,uint n){uint i=blockIdx.x*blockDim.x+threadIdx.x;if(i<n){int3 bin=calcGridPos(points[i]);hashes[i]=calcGridHash(bin);bins[3*i]=bin.x;bins[3*i+1]=bin.y;bins[3*i+2]=bin.z;}}
__global__ void doubleFieldProbe(float* output){output[0]=(float)(paramsD.pressure_height-1.0);}
