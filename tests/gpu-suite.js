...
export async function runGpuSuite(runtime,sources,{onCase=()=>{}}={}){
  ...
  await run('Original quadtree storage references preserve node and nested field mutations',async()=>{
    const source=await(await fetch('/tests/quadtree-storage-references.cu')).text(),options={valueBuffers:['nodes'],workgroupSize:[32]},write=await runtime.kernel(source,{...options,entry:'write_nodes'}),read=await runtime.kernel(source,{...options,entry:'read_nodes'}),nodes=runtime.createBuffer(1024),out=runtime.createBuffer(384);
    try{for(const kernel of [write,read]){runtime.batch().dispatch(kernel.bind({nodes,out}),[1]).submit();const values=await runtime.read(out);for(let i=0;i<32;i++)if(values.slice(i*3,i*3+3).join(',')!==[i+2,i+3,7].join(','))throw Error('Storage record reference mismatch '+i);}return {nodes:32,checkedValues:192,indexCapturedOnce:true,nestedMutationPersisted:true,originalClassBodies:true};}finally{runtime.destroyBuffer(nodes);runtime.destroyBuffer(out);}
  });
