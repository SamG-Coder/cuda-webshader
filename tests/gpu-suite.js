...
export async function runGpuSuite(runtime,sources,{onCase=()=>{}}={}){
  ...
  await run('Original bicubicTexture render kernels: GPU shader compilation only',async()=>{const source=await(await fetch('/tests/bicubic-texture-kernel.cuh')).text();for(const entry of ['d_render','d_renderBicubic','d_renderFastBicubic','d_renderCatRom'])await runtime.kernel(source,{entry,workgroupSize:[8,8,1]});return {scope:'Shader compilation only; no bicubic image correctness claim'};});
