...
export async function runGpuSuite(runtime,sources,{onCase=()=>{}}={}){
  ...
  await run('Original convolutionTexture shaders validate on GPU (compilation only)',async()=>{const source=await(await fetch('/tests/convolution-texture-kernel.cuh')).text();for(const entry of ['convolutionRowsKernel','convolutionColumnsKernel'])await runtime.kernel(source,{entry,workgroupSize:[16,12,1]});});
