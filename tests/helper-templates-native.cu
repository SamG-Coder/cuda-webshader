...
for(uint i=0;i<n;i++){float x=(int(i%13)-6)*0.25f,expected[]={x*x+x,x*3,2,x+1.5f};for(int j=0;j<4;j++)if(out[i*4+j]!=expected[j])return 1;uint v=0x80000000u+i;if(bits[i]!=v*v+v)return 1;}
for(uint i=n*4;i<out.size();i++)if(out[i]!=-12345)return 1;for(uint i=n;i<bits.size();i++)if(bits[i]!=0xdeadbeef)return 1;CHECK(cudaFree(o));CHECK(cudaFree(b));printf("n=%u PASS deduced templates, N-body float specialization, unsigned wraparound and guards\n",n);}return 0;}
