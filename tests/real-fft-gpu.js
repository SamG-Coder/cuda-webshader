export async function checkRealFFT(runtime){
 ...
 for(const [width,height] of [[1,1],[2,4],[8,4],[64,32],[512,512]]){
  const stride=2*(Math.floor(width/2)+1),count=width*height,size=stride*height,prefix='real-fft-'+width+'x'+height,input=await load(prefix+'-input'),spectrum=await load(prefix+'-spectrum'),nativeInverse=await load(prefix+'-inverse'),guarded=new Float32Array(size+8).fill(-77);guarded.set(input);
  ...
  try{
   ...
   if(normalizedRoundtripError>2e-6||roundtrip.slice(size).some(v=>v!==-77))throw Error('Real FFT roundtrip failed '+normalizedRoundtripError);
