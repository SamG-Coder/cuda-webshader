...
test('Logical warp indexing preserves a shadowed warpSize local',()=>{const s=source.replace('int lane=tile.thread_rank();','const int warpSize=16;int lane=tile.thread_rank();'),a=compile(s,{workgroupSize:[128]});assert.match(a.wgsl,/cw_thread.x \/ u32\(v_warpSize\)/);assert.throws(()=>compile(phases.replace('const int warp=','const int warpSize=16;const int warp='),{workgroupSize:[128]}),/complete tile branch/);});
