...
export async function check(row,runtime){
 ...
 const actual=await runtime.read(resources[f.out],f.expected.constructor);let maxError=0,failures=0;
 actual.forEach((v,i)=>{const err=Math.abs(v-f.expected[i]);maxError=Math.max(maxError,err);if(!Number.isFinite(v)||err>3e-6)failures++;});
 return {pass:failures===0,values:actual.length,maxError,failures,output:Array.from(actual.slice(0,128))};
