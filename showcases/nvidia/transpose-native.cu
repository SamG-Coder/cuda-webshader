...
int main() {
 ...
 for(int variant=0;variant<2;variant++)for(const auto& shape:shapes){
  ...
  else transposeNoBankConflicts<<<dim3(width/32,height/32),dim3(32,16)>>>(b,a,width,height);
