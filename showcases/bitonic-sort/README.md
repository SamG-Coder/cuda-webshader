...
Nested expression macros are bounded to eight levels and 65,536 AST nodes, with recursion rejected. Shared-array references capture their element indices and retain sequential alias semantics. Other unsupported pointer operations remain rejected. The original odd-even merge implementation is available as a separate [sandbox showcase](../../sandbox.html?example=odd-even-sort). This showcase implements the complete bitonic path.
