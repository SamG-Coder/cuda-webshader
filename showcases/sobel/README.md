...
functions from NVIDIA CUDA Samples commit `5443602d89ed99aede2e4b7bf329daddeadb320e`,
...
the same byte race-free. Wider pointer cast alignment remains unsupported.
...
`SobelShared` is still in scope but is not in this preset. It needs module-scope
dynamic shared byte storage and checked aligned casts back to `uchar4*`.
