...
export function inferTextureTypes(functions,kernel,walk,fail){
 ...
 for(const f of functions.filter(f=>f.qualifier!=='__global__'||f===kernel))walk(f.body,n=>{
  ...
  if(['tex1D','tex1Dfetch','tex2D','tex3D','tex2DLayered'].includes(name)){const p=n.args[0]?.kind==='id'?params.get(n.args[0].name):null;if(p)uses.push([p,name==='tex3D'&&n.callee.templateArgument==='float4'?'tex3Dfloat4':name==='tex1Dfetch'?'fetch_'+n.callee.templateArgument:name==='tex2D'&&['float2','float4'].includes(n.callee.templateArgument)?'tex2D'+n.callee.templateArgument:name==='tex2D'&&['uchar','unsigned char'].includes(n.callee.templateArgument)?'tex2Duchar':name,n]);return;}
...
export function textureShape(sampling){return {dimension:sampling==='tex2DLayered'?'2d-array':['tex3D','tex3Dfloat4'].includes(sampling)?'3d':'2d',format:sampling==='tex2Dfloat2'?'rg32float':sampling==='tex2Duchar'?'r8uint':sampling==='fetch_uint'?'r32uint':(sampling==='tex3Dfloat4'||sampling==='tex1D'||sampling==='tex2Dfloat4'||sampling==='tex2DLayered')?'rgba32float':sampling==='tex2D'?'r32float':'r8unorm'};}
