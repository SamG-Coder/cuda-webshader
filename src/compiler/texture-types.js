...
export function textureShape(sampling){return {dimension:['tex2DLayered','texCubemap'].includes(sampling)?'2d-array':['tex3D','tex3Dfloat4'].includes(sampling)?'3d':'2d',format:['tex2Dfloat2','fetch_float2'].includes(sampling)?'rg32float':sampling==='tex2Duchar'?'r8uint':['fetch_uint','tex2Duint'].includes(sampling)?'r32uint':(sampling==='tex3Dfloat4'||sampling==='tex1D'||sampling==='tex2Dfloat4'||sampling==='tex2DLayered'||sampling==='fetch_float4')?'rgba32float':['tex2D','texCubemap'].includes(sampling)?'r32float':'r8unorm'};}
