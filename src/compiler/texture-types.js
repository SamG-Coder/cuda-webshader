...
export function inferTextureTypes(functions,kernel,walk,fail){
 ...
 for(const f of functions.filter(f=>f.qualifier!=='__global__'||f===kernel))walk(f.body,n=>{
  ...
  if(['tex1D','tex2D','tex3D'].includes(name)){const p=n.args[0]?.kind==='id'?params.get(n.args[0].name):null;if(p)uses.push([p,name==='tex2D'&&n.callee.templateArgument==='float4'?'tex2Dfloat4':name,n]);return;}
...
export function textureShape(sampling){return {dimension:sampling==='tex3D'?'3d':'2d',format:(sampling==='tex1D'||sampling==='tex2Dfloat4')?'rgba32float':sampling==='tex2D'?'r32float':'r8unorm'};}
