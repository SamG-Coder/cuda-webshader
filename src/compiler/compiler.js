...
class Emitter {
  ...
  call(n) {
    ...
    if(name==='tex2D'){const rgba=n.callee.templateArgument==='float4';if(!['float','float4'].includes(n.callee.templateArgument)||n.args.length!==3||n.args[0].kind!=='id')this.fail('tex2D supports a bound float or float4 texture and two numeric coordinates.',n);const texture=this.lookup(n.args[0].name,n.args[0]),coords=n.args.slice(1).map(a=>this.expr(a));if(texture.kind!=='texture'||texture.dimension!=='2d'||texture.format!==(rgba?'rgba32float':'r32float')||coords.some(c=>!['f32','i32','u32','cw_uchar'].includes(c.type)))this.fail('tex2D requires a matching kernel texture parameter and numeric coordinates.',n);this[rgba?'rgba2DSamplingUsed':'float2DSamplingUsed']=true;return this.result(n,rgba?'vec4<f32>':'f32',`cw_sample_${rgba?'rgba':'float'}2d(${texture.code}, ${texture.sampler}, vec2<f32>(${coords.map(c=>this.convert(c.code,c.type,'f32',n)).join(', ')}), ${texture.coordinateScale}, ${texture.pixelPoint})`,coords.flatMap(c=>c.pre));}
    if(name==='tex1Dfetch'){
    ...
    if(name==='tex3D'){if(n.callee.templateArgument!=='float'||n.args.length!==4||n.args[0].kind!=='id')this.fail('tex3D supports a bound texture object and three float coordinates, returning float.',n);const texture=this.lookup(n.args[0].name,n.args[0]);if(texture.kind!=='texture'||texture.dimension!=='3d')this.fail('tex3D requires a kernel texture parameter.',n);const coords=n.args.slice(1).map(a=>this.expr(a));if(coords.some(c=>c.type!=='f32'))this.fail('tex3D coordinates must be floats.',n);this.float3DSamplingUsed=true;return this.result(n,'f32',`cw_sample_float3d(${texture.code}, ${texture.sampler}, vec3<f32>(${coords.map(c=>c.code).join(', ')}), ${texture.coordinateScale}, ${texture.pixelPoint})`,coords.flatMap(c=>c.pre));}
  ...
  emit() {
    ...
    for (const p of this.kernel.params) {
      ...
      if (p.pointer) {
        ...
        scalars.push({name:p.name,type:p.type==='cw_short'?'i32':['bool','cw_uchar4','cw_ushort'].includes(p.type)?'u32':p.type,...(['bool','cw_uchar4','cw_short','cw_ushort'].includes(p.type)?{sourceType:p.type}:{}),offset:scalars.length*4});
...
function instantiateHelperTemplates(ast, kernel) {
  ...
  function process(fn){
    ...
    walk(fn.body,node=>{
      ...
      if(['tex3D','tex1D','tex1Dfetch','tex2D'].includes(node.callee.name))return;
