...
class Emitter {
  ...
  emit() {
    ...
    for (const p of this.kernel.params) {
      ...
      if (p.pointer) {
        if(p.type==='cw_uchar'&&this.usage.writes.has(p.name))this.fail('Byte pointers currently support read-only packed storage; byte writes require an atomic packed-byte ABI.',p);
