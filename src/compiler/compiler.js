...
class Emitter {
  ...
  emit() {
    ...
    const emitHelpers=()=>{while(emittedHelpers<this.helpers.length){const helper=this.helpers[emittedHelpers++];
      ...
      helperLines.push(`fn f_${helper.name}(${[...helper.params.filter(p=>p.type!=='thread-block').map(p => p.type==='texture3d'?`cw_texture_${p.name}: texture_${textureShape(p.textureSampling).dimension}<f32>, cw_sampler_${p.name}: sampler${textureShape(p.textureSampling).format==='r32float'?`, cw_scale_${p.name}: vec2<f32>, cw_point_${p.name}: f32`:''}`:p.pointer?`cw_buffer_arg_${helper.params.indexOf(p)}: i32`:`${p.reference||p.constant?'v_':'cw_arg_'}${p.name}: ${p.reference?`ptr<function, ${p.type}>`:p.type}`),'cw_thread: vec3<u32>','cw_block: vec3<u32>','cw_grid: vec3<u32>'].join(', ')})${helper.result === 'void' ? '' : ` -> ${helper.result}`} {`,...indent(helper.params.filter(p=>p.pointer).map(p=>`var cw_buffer_offset_${helper.params.indexOf(p)}: i32 = cw_buffer_arg_${helper.params.indexOf(p)};`)),...indent(helper.params.filter(p=>p.type!=='thread-block'&&p.type!=='texture3d'&&!p.pointer&&!p.reference&&!p.constant).map(p=>`var v_${p.name}: ${p.type} = cw_arg_${p.name};`)), ...indent(body), '}');
    }};
    emitHelpers();
    ...
    const textureScales=textures.filter(t=>t.format==='r32float').map((t,i)=>({name:t.name,offset:scalars.length*4+i*12,pointOffset:scalars.length*4+i*12+8})),uniformBytes=scalars.length*4+textureScales.length*12,uniformSize=uniformBytes?Math.ceil(uniformBytes/16)*16:0;
    if(uniformSize){
      header.push('struct CWParams {',...scalars.map(s=>`  ${s.field||'p_'+s.name}: ${s.type},`));
      for(const scale of textureScales)header.push(`  cw_tex_${scale.name}_sx: f32,`,`  cw_tex_${scale.name}_sy: f32,`,`  cw_tex_${scale.name}_point: f32,`);
