...
class Emitter {
  ...
  bindReferenceHelper(helper,args,n){
    const storageRoots=helper.params.map((p,i)=>p.reference&&!p.constant&&args[i].rootSymbol?.referenceSpace==='storage'&&args[i].type===args[i].rootSymbol.type&&(args[i].storageReferenceIndexCode!==undefined||args[i].code===args[i].rootSymbol.code&&args[i].rootSymbol.storageReferenceIndexCode!==undefined)?args[i].rootSymbol.storageReferenceRoot||args[i].rootSymbol.code:null);
    ...
    if(spaces.every((space,i)=>space===null||space===(helper.params[i].referenceSpace||'function'))&&storageRoots.every((root,i)=>root===(helper.params[i].boundReferenceStorage||null))&&helper.params.every((p,i)=>!p.reference||args[i].sharedReferenceIndexCode===undefined&&!args[i].rootSymbol?.sharedReferenceRoot))return helper;
    ...
    const origin=helper.referenceOrigin||helper.name,key=JSON.stringify([origin,spaces,roots,storageRoots]);
  ...
  call(n) {
    ...
    if(name==='rintf'){if(args.length!==1||args[0].type!=='f32')this.fail('rintf requires one float argument.',n);this.roundEvenUsed=true;return this.result(n,'f32',`cw_round_even(${args[0].code})`,pre);}
    ...
    const unary = {sinf: 'sin', cosf: 'cos', tanf: 'tan', tan: 'tan', sqrtf: 'sqrt', rsqrtf: 'inverseSqrt', expf: 'exp', __expf:'exp', exp2f: 'exp2', logf: 'log', __logf:'log', log2f: 'log2', fabs:'abs', fabsf: 'abs', floorf: 'floor', ceilf: 'ceil', truncf: 'trunc'};
    ...
    if(this.overloads.has(name)){const matches=this.overloads.get(name).filter(f=>args.length<=f.params.length&&f.params.every((p,i)=>i>=args.length?p.defaultValue!==undefined:(typeName(p.pointer?(isArray(args[i].type)?args[i].type.element:null):args[i].type)===typeName(p.type)||String(p.type).startsWith('cw_objectptr_')&&['0i','0u'].includes(args[i].code)||String(p.type).startsWith('cw_objectlist_')&&args[i].rootSymbol?.objectImport?.type===p.type.replace('cw_objectlist_','cw_objectptr_')||String(p.type).startsWith('cw_bufferref_')&&args[i].rootSymbol?.objectImport?.type===p.type.replace('cw_bufferref_',''))));if(!matches.length){const convertible=this.overloads.get(name).filter(f=>f.classConstructor&&args.length===f.params.length&&f.params.every((p,i)=>typeName(args[i].type)===typeName(p.type)||String(args[i].type).startsWith('cw_objectptr_')&&this.structs.get('cw_struct_'+args[i].type.slice(13))?.base===String(p.type).slice(13)||!p.pointer&&!p.reference&&(numeric(args[i].type)||args[i].type==='cw_f64')&&(numeric(p.type)||p.type==='cw_f64')));if(convertible.length===1)matches.push(convertible[0]);}if(matches.length!==1)this.fail('Overload '+name+' requires one exact parameter-type match or an unambiguous scalar constructor conversion.',n);helper=matches[0];}
    ...
    const codes=args.map((a,i)=>{const p=helper.params[i];if(p.type==='texture3d'){const shape=textureShape(p.textureSampling),symbol=a.rootSymbol;if(symbol?.kind!=='texture'||symbol.dimension!==shape.dimension||symbol.format!==shape.format)this.fail('Texture helper argument requires the same inferred sampling format.',n.args[i]);return symbol.code+', '+symbol.sampler+(symbol.linearFetch?', '+symbol.lengthCode:'')+(['tex2D','tex2Dfloat2','tex2Dfloat4','tex3D','tex3Dfloat4'].includes(p.textureSampling)?', '+symbol.coordinateScale+', '+symbol.pixelPoint:'');}if(p.localPointer){if(references.has(a.rootSymbol))this.fail('Aliased local pointer/reference arguments are unsupported.',n.args[i]);references.set(a.rootSymbol,false);return a.pointerCode;}if(p.pointer)return a.pointerCode;if(p.type==='thread-block'){if(a.type!=='thread-block'||a.rootSymbol?.kind!=='thread-block')this.fail('thread_block arguments require a block handle.',n.args[i]);return null;}if(String(p.type).startsWith('cw_objectlist_')&&isArray(a.type)){const imported=a.rootSymbol?.objectImport;if(!imported||imported.type!==p.type.replace('cw_objectlist_','cw_objectptr_'))this.fail('Captured list arguments require a registered object pointer buffer.',n.args[i]);return `${imported.id*1048576}u + u32(${a.pointerCode||'0i'})`;}if(String(p.type).startsWith('cw_bufferref_')&&isArray(a.type)){const captured=bufferReferenceArgument(this,a,p.type,n);pre.push(...captured.pre.slice(a.pre.length));return captured.code;}if(!p.reference)return this.convert(a.code,a.type,p.type,n);
      ...
      if(p.boundReferenceStorage){if(a.type!==p.type||s?.constant)this.fail('Persistent reference requires a mutable object of the same type.',node);const index=a.storageReferenceIndexCode??s?.storageReferenceIndexCode;if(index===undefined)this.fail('Persistent object index is unavailable.',node);return `i32(${index})`;}
  ...
  emit() {
    ...
    const emitHelpers=()=>{while(emittedHelpers<this.helpers.length){const helper=this.helpers[emittedHelpers++];
      ...
      for (const p of helper.params) {
        ...
        if(p.boundReferenceStorage){const index='v_'+p.name,code=p.boundReferenceStorage+'['+index+']';p.symbol=this.add(p.name,{name:p.name,type:p.type,code,pointerCode:'&'+code,storageReferenceRoot:p.boundReferenceStorage,storageReferenceIndexCode:index,referenceSpace:'storage',constant:p.constant,atomic:false,kind:'reference'},p);continue;}
