...
class Emitter {
  ...
  emit() {
    ...
    for (const p of [...this.kernel.params,...this.deviceParams]) {
      ...
      if(p.type==='texture3d'){if(p.pointer)this.fail('Texture objects must be passed by value.',p);const binding=bufferCount+textures.length*2,samplerBinding=binding+1,sampling=p.textureSampling||'tex3D',{dimension,format}=textureShape(sampling);const symbol={name:p.name,type:p.type,code:'t_'+p.name,sampler:'s_'+p.name,coordinateScale:dimension==='3d'?`vec3<f32>(cw_params.cw_tex_${p.name}_sx, cw_params.cw_tex_${p.name}_sy, cw_params.cw_tex_${p.name}_sz)`:`vec2<f32>(cw_params.cw_tex_${p.name}_sx, cw_params.cw_tex_${p.name}_sy)`,pixelPoint:`cw_params.cw_tex_${p.name}_point`,...(sampling.startsWith('fetch_')?{linearFetch:sampling.slice(6),lengthCode:`cw_params.cw_tex_${p.name}_length`}:{}),dimension,format,kind:'texture',constant:true};this.add(p.name,symbol,p,true);p.symbol=symbol;textures.push({name:p.name,binding,samplerBinding,dimension,format,...(sampling==='texCubemap'?{coordinates:'cube-direction'}:{}),...(['tex2Duchar','tex2Duint'].includes(sampling)?{coordinates:sampling==='tex2Duint'?'pixel-uint':'pixel-byte'}:['tex2Dfloat2','tex2Dfloat4'].includes(sampling)?{coordinates:'2d'}:sampling.startsWith('fetch_')?{coordinates:'linear'}:{})});header.push(`@group(0) @binding(${binding}) var ${symbol.code}: texture_${dimension.replace('-','_')}<${['r32uint','r8uint'].includes(format)?'u32':'f32'}>;`,`@group(0) @binding(${samplerBinding}) var ${symbol.sampler}: sampler;`);continue;}
      if (p.pointer) {
