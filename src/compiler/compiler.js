...
class Emitter {
  ...
  call(n) {
    ...
    if(this.overloads.has(name)){const matches=this.overloads.get(name).filter(f=>args.length<=f.params.length&&f.params.every((p,i)=>i>=args.length?p.defaultValue!==undefined:(typeName(p.pointer?(isArray(args[i].type)?args[i].type.element:null):args[i].type)===typeName(p.type)||String(p.type).startsWith('cw_objectptr_')&&['0i','0u'].includes(args[i].code)||String(p.type).startsWith('cw_objectlist_')&&args[i].rootSymbol?.objectImport?.type===p.type.replace('cw_objectlist_','cw_objectptr_'))));if(!matches.length){const convertible=this.overloads.get(name).filter(f=>f.classConstructor&&args.length===f.params.length&&f.params.every((p,i)=>typeName(args[i].type)===typeName(p.type)||!p.pointer&&!p.reference&&numeric(args[i].type)&&numeric(p.type)));if(convertible.length===1)matches.push(convertible[0]);}if(matches.length!==1)this.fail('Overload '+name+' requires one exact parameter-type match or an unambiguous scalar constructor conversion.',n);helper=matches[0];}
