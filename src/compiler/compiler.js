  ...
  emit() {
    ...
    const emitHelpers=()=>{while(emittedHelpers<this.helpers.length){const helper=this.helpers[emittedHelpers++];
      ...
      for (const p of helper.params) {
        ...
        if(p.reference&&!numeric(p.type)&&!vectorLength(p.type)&&!(p.constant&&(['cw_uchar2','cw_uchar4'].includes(p.type)||vectorLength(p.type)||this.structs.has(p.type))))this.fail('Helper references require numeric scalars or vectors.',p);
