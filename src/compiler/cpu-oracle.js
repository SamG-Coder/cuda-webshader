...
export function executeCPU(artifact,buffers,scalars,workgroups,{instructionBudget=1_000_000}={}){
  ...
  for(const p of [...artifact.kernel.params,...(artifact.ast.deviceGlobals||[]).filter(g=>g.symbol).map(g=>({...g,pointer:true}))]){
    ...
    if(p.pointer){if(p.kind==='device-global'&&buffers[p.name]?.byteLength<artifact.metadata.bindings.find(b=>b.name===p.name).minBindingSize)throw Error('Device global buffer too small.');if(p.type==='cw_uchar'&&!(buffers[p.name] instanceof Uint8Array))throw Error('Byte CPU buffers require Uint8Array.');if(!ArrayBuffer.isView(buffers[p.name]))throw new Error(`Missing CPU buffer ${p.name}.`);baseEnv.set(p.symbol,{value:new BufferView(buffers[p.name],p.type)});}
