...
export class GpuRuntime {
  ...
  createLinearTexture(data,{label='CUDA linear texture'}={}) {
    this.assertAlive();if(!(data instanceof Uint8Array)&&!(data instanceof Uint32Array))throw Error('Linear textures require normalized Uint8Array bytes or Uint32Array elements.');
    const limit=this.device.limits.maxTextureDimension2D;if(!data.length||data.length>Math.min(limit*limit,0x7fffffff))throw new RangeError('Linear texture length exceeds device dimensions.');
    const width=Math.min(data.length,limit),height=Math.ceil(data.length/width),format=data instanceof Uint32Array?'r32uint':'r8unorm',upload=new data.constructor(width*height);upload.set(data);
    const gpuTexture=this.device.createTexture({label,size:[width,height,1],dimension:'2d',format,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST});this.device.queue.writeTexture({texture:gpuTexture},upload,{bytesPerRow:width*data.BYTES_PER_ELEMENT,rowsPerImage:height},[width,height,1]);
...
export class ComputeBatch {
  ...
  dispatch(invocation,workgroups) {
    ...
    for(const surface of invocation.kernel.artifact.metadata.surfaces||[]){const r=invocation.buffers[surface.name],block=invocation.kernel.artifact.metadata.workgroupSize;if(!['global-xy','global-xyz'].includes(surface.coordinates)||groups[0]*block[0]>r.width||groups[1]*block[1]>r.height||groups[2]*block[2]>(surface.coordinates==='global-xyz'?r.depth:1))throw Error('Surface dispatch exceeds the checked global '+(surface.coordinates==='global-xyz'?'XYZ':'XY')+' extent.');}
