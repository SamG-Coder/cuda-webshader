...
export class GpuRuntime {
  ...
  createTexture2D(data,{width,height,filter='linear',addressMode='repeat',label='CUDA float texture'}={}){
    this.assertAlive();if(!(data instanceof Float32Array)||![width,height].every(n=>Number.isInteger(n)&&n>0&&n<=this.device.limits.maxTextureDimension2D)||data.length!==width*height||data.some(v=>!Number.isFinite(v)))throw new RangeError('2D texture requires finite floats matching width and height within device limits.');
    ...
    const gpuTexture=this.device.createTexture({label,size:[width,height,1],dimension:'2d',format:'r32float',usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST});this.device.queue.writeTexture({texture:gpuTexture},data,{bytesPerRow:width*4,rowsPerImage:height},[width,height,1]);const resource={id:++resourceId,runtime:this,owned:true,destroyed:false,gpuTexture,view:gpuTexture.createView(),sampler:this.device.createSampler({minFilter:filter,magFilter:filter,addressModeU:addressMode,addressModeV:addressMode}),format:'r32float',dimension:'2d',width,height,depth:1};this.textures.add(resource);this.stats.dataBytesUploaded+=data.byteLength;return resource;
  }
