...
export class GpuRuntime {
  ...
  createTexture3D(data,{width,height,depth,filter='linear',addressMode='repeat',label='CUDA 3D texture',format='r8unorm',storage=false}={}){
    this.assertAlive();if(!['r8unorm','r32float'].includes(format)||storage&&format!=='r32float')throw Error('Unsupported 3D texture format or storage format.');
    const bytes=format==='r32float'?4:1,Type=format==='r32float'?Float32Array:Uint8Array;
    ...
    if(data)this.device.queue.writeTexture({texture:gpuTexture},data,{bytesPerRow:width*bytes,rowsPerImage:height},[width,height,depth]);
    const resource={id:++resourceId,runtime:this,owned:true,destroyed:false,gpuTexture,view:gpuTexture.createView(),sampler:this.device.createSampler({minFilter:filter,magFilter:filter,addressModeU:addressMode,addressModeV:addressMode,addressModeW:addressMode}),format,dimension:'3d',width,height,depth,storage,filter};this.textures.add(resource);this.stats.dataBytesUploaded+=data?.byteLength||0;return resource;
  }
