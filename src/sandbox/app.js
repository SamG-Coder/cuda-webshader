...
function dispatchPreview(a){const batch=runtime.batch().dispatch(a.invocation,a.config.groups);for(const [target,source]of Object.entries(a.config.feedback||{}))batch.copy(a.buffers[source],a.buffers[target]);batch.submit();}
...
async function run(){
 ...
 try{
  ...
  const compiled=await compiler.compile(input.source,{entry:$('entry').value.trim()||undefined,workgroupSize:block,sharedMemoryBytes:Number($('shared-bytes').value),...(fullWorkgroups.length?{fullWorkgroups}:{})});
...
async function example(name){const paths={nbody:'showcases/nbody/kernel.cu',wave:'showcases/simplegl/kernel.cu',particles:'kernels/particles.cu',saxpy:'kernels/saxpy.cu'};let source;if(paths[name]){const response=await fetch(paths[name]);if(!response.ok)throw Error('Example could not be loaded.');source=await response.text();}else source='__global__ void fill(float* output, unsigned int n) {\n    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;\n    if (i < n) output[i] = (float)i;\n}\n';editor.setValue(source);await editor.getAction('editor.foldAllBlockComments')?.run();$('filename').textContent=paths[name]?.split('/').pop()||'untitled.cu';detect();if(name==='nbody'){$('entry').value='integrateBodies<float>';$('block').value='128,1,1';$('shared-bytes').value='2048';$('full-bounds').value='deviceNumBodies';presetConfig={groups:[4,1,1],scalars:{deviceOffset:0,deviceNumBodies:512,deltaTime:0.002,damping:0.999,numTiles:4,'constant.softeningSquared':0.125},buffers:{newPos:{records:512,fill:'zero'},oldPos:{records:512,fill:'sphere'},vel:{records:512,fill:'zero'}},output:'newPos',feedback:{oldPos:'newPos'}};log('NVIDIA N-body: original force and integration kernels; 512 bodies, whole blocks, fixed time step. Position feedback stays on the GPU.');}$('animate').checked=['wave','particles','nbody'].includes(name);await run();}
...
try{
 ...
 }else {const selected=new URLSearchParams(location.search).get('example')||'wave';if(!['wave','particles','nbody','saxpy','blank'].includes(selected))throw Error('Unknown sandbox example.');$('example').value=selected;await example(selected);}
