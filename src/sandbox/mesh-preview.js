...
import {vertexIndex,instanceIndex,positionLocal,color,mix,transformNormalToView} from 'three/tsl';
...
export async function meshPreview({owner,container,plan,runtime,...options}){
 const particles=plan.preview.kind==='particles';const renderer=owner.renderer=new THREE.WebGPURenderer({device:runtime.device,antialias:false});
 ...
 if(particles){bounds.expandByPoint(new THREE.Vector3(-1,-1,-1));bounds.expandByPoint(new THREE.Vector3(1,1,1));}
