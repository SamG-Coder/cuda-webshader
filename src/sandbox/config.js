...
export function validateConfig(config,metadata){
 ...
 if(config.image!==undefined){const v=config.image,b=metadata.bindings.find(b=>b.name===config.output);if(b.elementType!=='u32'||v?.format!=='rgba8'||![v.width,v.height].every(n=>Number.isInteger(n)&&n>0)||v.width*v.height!==config.buffers[config.output].records)throw Error('Image preview requires uint RGBA8 storage and matching width/height.');}
