...
export function validateConfig(config,metadata){
 ...
 for(const t of metadata.textures||[]){const spec=config.textures?.[t.name];if(!spec||!Array.isArray(spec.dimensions)||spec.dimensions.length!==3||spec.dimensions.some(n=>!Number.isInteger(n)||n<1||n>2048)||typeof spec.source!=='string'||!['linear','nearest'].includes(spec.filter)||!['repeat','clamp-to-edge','mirror-repeat'].includes(spec.addressMode))throw Error('Texture requires dimensions, a byte-data source, filter and addressMode.');bytes+=spec.dimensions.reduce((a,b)=>a*b,1);}
