...
export function validateConfig(config,metadata){
 ...
 if(config.image!==undefined){const v=config.image;if(v?.alpha!==undefined&&!['source','opaque'].includes(v.alpha))throw Error('Image alpha must be source or opaque.');const b=metadata.bindings.find(b=>b.name===config.output);if(b.elementType!=='u32'||!['rgba8','r8-in-u32'].includes(v?.format)||![v.width,v.height].every(n=>Number.isInteger(n)&&n>0)||v.width*v.height!==config.buffers[config.output].records)throw Error('Image preview requires uint RGBA8 or r8-in-u32 storage and matching width/height.');}
