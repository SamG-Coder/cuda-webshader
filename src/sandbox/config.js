...
export function validateConfig(config,metadata){
 ...
 if(config.image!==undefined){const v=config.image;if(v?.alpha!==undefined&&!['source','opaque'].includes(v.alpha))throw Error('Image alpha must be source or opaque.');const b=metadata.bindings.find(b=>b.name===config.output);if(!((b.elementType==='u32'&&['rgba8','r8-in-u32'].includes(v?.format))||(b.elementType==='f32'&&v?.format==='gray-f32'))||![v.width,v.height].every(n=>Number.isInteger(n)&&n>0)||v.width*v.height!==config.buffers[config.output].records)throw Error('Image preview requires uint RGBA8/r8-in-u32 or float gray-f32 storage and matching width/height.');}
...
export function preparePass(pass,metadata,config,rootMetadata){
 ...
 for(const b of [...(metadata.textures||[]),...(metadata.surfaces||[])]){const name=pass.bindings?.[b.name]??b.name,root=resources.find(r=>r.name===name);if(!root||root.format!==b.format||root.dimension!==b.dimension||(b.access==='write-only'&&root.access!=='write-only'))throw Error(`Pass ${pass.entry}: ${b.name} needs a matching existing texture or writable surface.`);bindings[b.name]=name;}
 ...
 validateConfig({groups:pass.groups,scalars,buffers,output:metadata.bindings[0]?.name},{...metadata,textures:[],surfaces:[]});
